...
class AIComplianceRAG:
    ...
    def generate_answer(self, query: str, history: List[Dict] = None) -> Tuple[str, str]:
        ...
        5. 대화 문맥 완벽 적용 🚨: `[이전 대화 기록]`을 반드시 분석하십시오. 사용자가 이전 대화에서 적용 국가(예: "한국에서 사용 예정"), 특정 기술, 산업 분야 등을 언급했다면, 이를 현재 질문의 '절대적인 전제 조건'으로 삼아 해당 국가/상황에 맞는 법률(예: 한국에서 사용 시 한국 인공지능기본법 중심)을 집중적으로 분석하여 답변하십시오.
        6. 용어 통일 지침: 한국의 법안을 지칭할 때는 반드시 '인공지능기본법'이라는 용어를 사용하십시오. '한국 AI 기본법', '국내 AI 법' 등의 표현 대신 '한국 인공지능기본법' 또는 '한국 인공지능 발전과 신뢰 기반 조성 등에 관한 기본법'으로 명칭을 통일하여 답변하십시오. '한국 인공지능기본법'은 약칭이며, '한국 인공지능 발전과 신뢰 기반 조성 등에 관한 기본법'은 정식 명칭입니다.

        [답변 구조 및 포맷 지침]
        아래 형식과 순서를 엄격히 지켜 답변하십시오. (주의: '본문', '국가 미지정 시' 같은 안내용 괄호나 단어는 화면에 절대 출력하지 마십시오.)
        ...
        💡 [국가 미지정 시 대응]: 대화 기록과 질문 어디에도 사용 국가나 시장이 특정되지 않았다면, 먼저 규제가 가장 엄격한 'EU AI Act' 기준으로 고위험 여부와 규제를 상세히 설명한 뒤, '한국 인공지능기본법' 등 타 관할권의 기준은 어떻게 다른지 비교하여 설명하십시오.
        💡 [국가 지정 시 대응]: 대화 기록이나 질문에 특정 국가가 명시되어 있다면(예: 한국), 다른 나라 법의 장황한 설명을 줄이고 해당 관할권의 법률을 집중적으로 적용하여 규제 대상 여부, 핵심 의무, 위반 리스크를 심층 분석하십시오.

        (줄바꿈 후 역질문 작성 - 필수)
        더욱 정확한 맞춤형 규제 리스크를 진단하기 위해, 추가로 확인이 필요한 사항은 다음과 같습니다.
        * (대화 기록을 포함하여 국가가 전혀 파악되지 않은 경우 반드시 1순위로 질문: "해당 AI 시스템을 주로 어느 국가(시장)에서 서비스하실 계획인가요?")
        * (구체적인 확인 사항 1 - 반드시 '~인가요?', '~합니까?' 등 자연스러운 대화형 종결어미 사용)
        * (구체적인 확인 사항 2 - 반드시 '~인가요?', '~합니까?' 등 자연스러운 대화형 종결어미 사용)
